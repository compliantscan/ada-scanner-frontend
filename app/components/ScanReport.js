...
function renderFixDetails(item) {
  ...
  if (item.fixType === 'color-contrast' && item.fix?.manualReviewRequired) {
    return (
      <div style={{ padding: '14px', borderRadius: '8px', background: '#f3f4f6', borderLeft: '3px solid #9ca3af', color: '#6b7280' }}>
  ...
  return (
    <div className="ss-backdrop" role="dialog" aria-modal="true" aria-label="Unlock full report" onClick={onClose}>
      <div className="ss-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <button className="ss-back-divider back-to-report-mobile" onClick={onClose}>← Back to report</button>
      </div>
    </div>
  );
}

export default function ScanReport({ result, variant = 'public', title }) {
  ...
  async function downloadPdf() {
    ...
    try {
      const response = await fetch(`${apiUrl}/lead-report/pdf`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ scanId: report.id, scanAccessKey: report.scanAccessKey }),
      });
      if (!response.ok) {
      ...
      a.href = url;
      a.download = `ada-full-report-${report.id}.pdf`;
      a.click();
  ...
  const pageTitle = title || (variant === 'dashboard' ? 'Dashboard scan report' : variant === 'paid' ? 'Paid scan report' : 'Scan results');
  ...
  const totalAffectedElements = Number.isFinite(severityCounts.totalAffectedElements)
    ...
    : (report?.violations || []).reduce((sum, violation) => sum + (Number.isFinite(violation.affectedElements) ? violation.affectedElements : 0), 0);
  ...
  const lockedViolations = unlocked ? [] : (report.violations || []).slice(3);
  const expires = report.expiresAt ? new Date(report.expiresAt).toLocaleDateString() : '7 days';
    ...
    <>
      <nav className="nav">
        <div className="nav-inner">
          ...
          <button className="nav-hamburger" onClick={() => setMenuOpen(!menuOpen)} aria-label={menuOpen ? 'Close menu' : 'Open menu'} aria-expanded={menuOpen}>
            <span className="hamburger-icon">{menuOpen ? '✕' : '☰'}</span>
          </button>
        </div>
      </nav>

      <div className={`mobile-menu-backdrop ${menuOpen ? 'active' : ''}`} onClick={() => setMenuOpen(false)} aria-hidden="true" />
              ...
              <table>
                ...
                <tbody>
                  {(report.pages || []).map(page => {
                    const fallbackAffectedElements = (report?.violations || []).reduce((sum, violation) => sum + (Number.isFinite(violation.affectedElements) ? violation.affectedElements : 0), 0);
                    const affectedElements = Number.isFinite(page.affectedElements) ? page.affectedElements : fallbackAffectedElements;
        ...
        {unlocked && hasViolations && (
          <section className="paid-tools-panel">
            ...
            <div className="badge-code">
              ...
              <pre><code>{report.complianceBadgeHtml || `<a href="${escapeAttribute(report.url)}" aria-label="View accessibility status"><img src="${escapeAttribute(report.url)}/ada-compliance-badge.svg" alt="ADA accessibility monitoring enabled"></a>`}</code></pre>
            </div>
        ...
        {hasViolations && (
          <section className="trailer-section">
            <div className="section-row">
              ...
              <button type="button" className="locked-note locked-cta" onClick={() => { setModalOpen(false); setShowEmailGate(true); }}>
          ...
          <>
            ...
            <div className={`paywall-modal${modalOpen ? ' open' : ''}`} onClick={(e) => e.stopPropagation()}>
              <div className="paywall-cards-grid">
                {PLANS.map((plan, i) => (
                  <div key={plan.name} className={`pricing-drawer-card${plan.featured ? ' featured' : ''}`} style={{ transitionDelay: `${i * 0.1}s` }}>
                    ...
                    <button className="plan-cta" onClick={() => { setModalOpen(false); setShowEmailGate(true); }}>
                      Start monitoring
                    </button>
              ...
              <button className="secondary-button back-to-report-mobile" onClick={() => setModalOpen(false)}>← Back to report</button>
            </div>
          </>
        )}
